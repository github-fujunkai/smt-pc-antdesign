...
const App = () => {
  ...
  const fetchData = () => {
    setLoadingSave(true)
    http.post(config.API_PREFIX + api.barcodeReaderAll, {
    }).then((res) => {
      console.log(res)
  ...
  const onFinish = (values) => {
    ...
    http.post(config.API_PREFIX + api.barcodeReaderSaveOrUpdate, {
      separator: {
        ...separator,
  ...
  const handleDeleteItem = (index, remove) => {
    if (itemContrasts[index]) {
      ...
      http.del(config.API_PREFIX + api.barcodeReader + `/${itemContrasts[index].id}`, {
      }).then((res) => {
        console.log(res)
        remove(index)
