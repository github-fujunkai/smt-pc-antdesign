...
const App = () => {
  ...
  const { components, resizableColumns, tableWidth, resetColumns } = useAntdResizableHeader({
    ...
    columnsState: {
      persistenceKey: 'localKeyInfoScanning',
  ...
  const fetchData = () => {
    ...
    const {factoryId, workshopId, areaId, programName, deviceOrderNum, tableNo, slot, division, itemCode, operationResult, createBy, startTime, endTime} = formSearch.getFieldsValue()
    if (factoryId) {
    ...
    http.post(config.API_PREFIX + api.pdaScanLogPage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  const exportData = () => {
    ...
    http.post(config.API_PREFIX + api.pdaScanLogExportData + `?${query}`, ).then((res) => {
      message.success('导出成功！')
      downloadCSV(res, '信息查询-扫描记录导出-CSV文件')
...
*/
  ...
  const saveColumnsOrder = (columns) => {
    localStorage.setItem('columnsOrderInfoScanning', JSON.stringify(columns.map(col => col.key)));
  };

  useEffect(() => {
    const savedOrder = localStorage.getItem('columnsOrderInfoScanning');
    if (savedOrder) {
  ...
  function refreshPage() {
    localStorage.removeItem('columnsOrderInfoScanning')
        ...
        <div className="search-wrapper" style={{display: isShowSearch ? 'block' : 'none'}}>
          ...
          >
            <Row gutter="24">
              ...
              <Col span={8}>
                <Form.Item label="台车" name="tableNo">
                  <Select
                    placeholder="请选择"
                    allowClear
                    options={dictBaseAll?.tableNo?.map(item => ({
                      value: item.dictKey,
                      label: item.dictValue,
                    }))}
                  />
              ...
              <Col span={8}>
                <Form.Item label="料站" name="slot">
                  <Select
                    placeholder="请选择"
                    allowClear
                    options={dictBaseAll?.slot?.map(item => ({
                      value: item.dictKey,
                      label: item.dictValue,
                    }))}
                  />
              ...
              <Col span={8}>
                <Form.Item label="道数" name="division">
                  <Select
                    placeholder="请选择"
                    allowClear
                    options={dictBaseAll?.division?.map(item => ({
                      value: item.dictKey,
                      label: item.dictValue,
                    }))}
                  />
              ...
              <Col span={8}>
                <Form.Item label="员工" name="createBy">
                  <Select
                    placeholder="请选择"
                    allowClear
                    options={dictBaseAll?.optUser?.map(item => ({
                      value: item.dictKey,
                      label: item.dictValue,
                    }))}
                  />
