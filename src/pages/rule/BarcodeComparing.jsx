...
const App = () => {
  ...
  const del = (record) => {
    confirm({
      ...
      onOk() {
        console.log('OK');
        http
          .del(config.API_PREFIX + 'item/contrast/parent' + `/${record?.id}`, {})
          .then((res) => {
            fetchData();
  ...
  const onFinishComparing = (values) => {
    ...
    formComparing.validateFields().then((values) => {
      ...
      http
        .post(config.API_PREFIX + 'item/contrast/parent/saveOrUpdate', params)
        .then((res) => {
          console.log(res);
          message.success(msg);
  ...
  const handleDeleteItem = (index, remove) => {
    ...
    if (itemList[index]) {
      ...
      http
        .del(config.API_PREFIX + 'item/contrast' + `/${itemList[index].id}`, {})
        .then((res) => {
          console.log(res);
          remove(index);
        ...
        <div className="search-wrapper">
          <Form form={formSearch} onFinish={onFinish}>
            <Row gutter="24">
              ...
              <Col span={8}>
                <Form.Item label="料号位置" name="startPos">
                  <Input allowClear placeholder="请输入" />
                </Form.Item>
              </Col>
