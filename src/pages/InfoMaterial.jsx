...
const App = () => {
  ...
  const { components, resizableColumns, tableWidth, resetColumns } = useAntdResizableHeader({
    ...
    columnsState: {
      persistenceKey: 'localKeyInfoMaterial',
  ...
  const fetchData = () => {
    ...
    const {factoryId, workshopId, areaId, itemCode, supplierName, itemSn, startTime, endTime} = formSearch.getFieldsValue()
    if (factoryId) {
    ...
    http.post(config.API_PREFIX + api.itemHistoryPage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  const exportData = () => {
    ...
    http.post(config.API_PREFIX + api.itemHistoryExportData + `?${query}`, ).then((res) => {
      message.success('导出成功！')
      downloadCSV(res, '信息查询-导出物料记录-CSV文件')
...
*/
  ...
  const saveColumnsOrder = (columns) => {
    localStorage.setItem('columnsOrderInfoMaterial', JSON.stringify(columns.map(col => col.key)));
  };

  useEffect(() => {
    const savedOrder = localStorage.getItem('columnsOrderInfoMaterial');
    if (savedOrder) {
  ...
  function refreshPage() {
    localStorage.removeItem('columnsOrderInfoMaterial')
