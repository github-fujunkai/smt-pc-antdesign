...
const App = () => {
  ...
  const handleOk = () => {
    formCreate.validateFields().then(values => {
      ...
      setLoadingOk(true)
      const { mainItem, replaceItem } = values
      
      http.post(config.API_PREFIX + api.itemReplaceCurrency, {
        mainItem,
        replaceItem,
        isValid: 'Y', // todo 默认咋搞？
      }).then((res) => {
  ...
  */
  const del = (record) => {
    confirm({
      ...
      onOk() {
        http.del(config.API_PREFIX + api.itemReplaceCurrency + `/${record.id}`, {}).then((res) => {
          fetchData()
  ...
  const fetchData = () => {
    ...
    const {mainItem, replaceItem} = formSearch.getFieldsValue()
    if (mainItem) {
    ...
    http.post(config.API_PREFIX + api.itemReplaceCurrencyPage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  const loadTemplate = () => {
    ...
    http.post(config.API_PREFIX + api.itemReplaceCurrencyLoadTemplate, {
    }).then((res) => {
  ...
  const handleCustomRequest = ({ file, onSuccess, onError }) => {
    ...
    http.post(config.API_PREFIX + api.itemReplaceCurrencyImportData, formData).then((res) => {
  ...
  const exportData = () => {
    ...
    http.post(config.API_PREFIX + api.itemReplaceCurrencyExportData + `?${query}`, ).then((res) => {
      message.success('导出成功！')
      downloadCSV(res, '通用替代料导出-CSV文件')
  ...
  return (
    <div className="content-wrapper">
      ...
      <div className="content">
        ...
        <div className="table-wrapper">
          <div style={{marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
            ...
            <Button type="primary" icon={<PlusOutlined />}  onClick={showModal}>新增通用替代料</Button>
          </div>
      ...
      <Modal confirmLoading={loadingOk} title="新增通用替代料" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
        <Form
          labelCol={{ span: 5 }}
          style={{padding: 32}}
          form={formCreate}
