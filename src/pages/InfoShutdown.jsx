...
const App = () => {
  ...
  const { components, resizableColumns, tableWidth, resetColumns } = useAntdResizableHeader({
    ...
    columnsState: {
      persistenceKey: 'localKeyInfoShutdown',
  ...
  const fetchData = () => {
    ...
    const {factoryId, workshopId, areaId, deviceOrderNum, lane, isResrt, startTime, endTime} = formSearch.getFieldsValue()
    if (factoryId) {
    ...
    http.post(config.API_PREFIX + api.stopInfoPage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  const exportData = () => {
    ...
    http.post(config.API_PREFIX + api.stopInfoExportData + `?${query}`, ).then((res) => {
      message.success('导出成功！')
      downloadCSV(res, '信息查询-导出停机信息-CSV文件')
...
*/
  ...
  const saveColumnsOrder = (columns) => {
    localStorage.setItem('columnsOrderInfoShutdown', JSON.stringify(columns.map(col => col.key)));
  };

  useEffect(() => {
    const savedOrder = localStorage.getItem('columnsOrderInfoShutdown');
    if (savedOrder) {
  ...
  function refreshPage() {
    localStorage.removeItem('columnsOrderInfoShutdown')
        ...
        <div className="search-wrapper" style={{display: isShowSearch ? 'block' : 'none'}}>
          ...
          >
            <Row gutter="24">
              ...
              {/* <Col span={8}>
                <Form.Item label="类型" name="stopType">
                  <Select
                    placeholder="请选择"
                    allowClear
                    // options={dictBaseAll?.stopType?.map(item => ({
                    //   value: item.dictKey,
                    //   label: item.dictValue,
                    // }))}
                    options={[{
              ...
              <Col span={8}>
                <Form.Item label="复位" name="isResrt">
                  <Select
                    placeholder="请选择"
                    allowClear
                    options={[{
