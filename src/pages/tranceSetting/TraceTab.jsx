...
const App = (props) => {
  ...
  useEffect(() => {
    ...
    const codeReadersArr = traceConfig?.codeReaders?.map(itemArray => ({
      num: itemArray[0].num,
      list: itemArray.map(({ isValid, ...rest }) => ({
          ...rest,
          isValid: isValid === 'Y'
      }))
  }));
  ...
  const onFinish = (values) => {
    ...
    let codeReadersArr = JSON.parse(JSON.stringify(values?.codeReaders || []))
    // const codeReaders = codeReadersArr.flatMap((item) =>
    //   item.list
    //     ? item.list.map((subItem) => ({
    //         num: item.num,
    //         ...subItem,
    //         isValid: subItem.isValid ? 'Y' : 'N',
    //       }))
    //     : item
    // );

    const codeReaders = codeReadersArr.map(item => item.list.map(subItem => ({
      ...subItem,
      isValid: subItem.isValid ? 'Y' : 'N',
      num: item.num,
    })));
    console.log('codeReaders', codeReaders)
        ...
        <Form.List name="codeReaders">
          {(fields, { add, remove }) => (
            <div style={{ display: 'flex', rowGap: 16, flexDirection: 'column' }}>
              {fields.map((field) => {
                const num = form.getFieldValue([field.name, 'num']);
                return (
                  <Card
                    size="small"
