  ...
  const del = (record) => {
    confirm({
      ...
      onOk() {
        ...
        http
          .del(
            config.API_PREFIX + api.basicTestDefectCode + `/${record?.id}`,
            {}
          )
          .then((res) => {
            fetchData();
  ...
  const handleOk = () => {
    formCreate
      ...
      .then((values) => {
        ...
        setLoadingOk(true);
        // wtf
        const { defectCategory, defectCode, defectName } = values;
        let params = { defectCategory, defectCode, defectName };
        let action = null;
        let msg = "";
        let apiUrl = "";
        ...
        if (activeId !== -1) {
          ...
          apiUrl = `${config.API_PREFIX}${api.basicTestDefectCode}`;
          params.id = activeId;
          msg = "修改成功！";
        } else {
          action = http.post;
  ...
  */
  ...
  useEffect(() => {
    ...
    http
      ...
      http
      .get(config.API_PREFIX + 'prod/repair/type/test/code', {})
      .then((res) => {
        ...
        setTestCode(prevVariable => {
          const newVariable = prevVariable.concat(data);
          return newVariable;
        });
      })
      .catch((err) => {
  ...
  const fetchData = () => {
    ...
    const { defectCategory, defectCode, defectName } =
    ...
    http
      .get(config.API_PREFIX + api.basicTestDefectCodePage, params)
      .then((res) => {
        console.log("res", res);
        const data = res?.bizData;
