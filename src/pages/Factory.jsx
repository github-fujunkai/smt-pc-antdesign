...
import { Typography, Checkbox, Radio, Upload, Table, Modal, Breadcrumb, Form, Row, Col, Switch, Input, Space, Button, message } from 'antd';
...
const App = () => {
  ...
  const { components, resizableColumns, tableWidth, resetColumns } = useAntdResizableHeader({
    ...
    columnsState: {
      persistenceKey: 'localKeyFactory',
  ...
  const del = (record) => {
    confirm({
      ...
      onOk() {
        console.log('OK');
        http.del(config.API_PREFIX + api.areaFactory + `/${record?.id}`, {}).then((res) => {
          fetchData()
  ...
  const showModal = (action, record) => {
    if (action === 'update' && record) {
      const { id, areName, areSn, isValid } = record
        activeId = record?.id
        formCreate.setFieldsValue({
          id, areName, areSn, isValid: isValid === 'Y' ? true : false,
        })
      } else {
        activeId = -1
  ...
  const handleOk = () => {
    formCreate.validateFields().then(values => {
      ...
      setLoadingOk(true)
      const { id, areName, areSn, isValid } = values
      let params = { id, areName, areSn, isValid: isValid ? 'Y' : 'N', }
      let action = null
      let msg = ''
      if (activeId !== -1) {
        params.id = activeId
      ...
      action(config.API_PREFIX + api.areaFactory, params).then((res) => {
        formCreate.resetFields();
        setLoadingOk(false)
  ...
  const fetchData = () => {
    ...
    http.post(config.API_PREFIX + api.areaFactoryPage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  return (
    <div className="content-wrapper">
      ...
      <div className="content">
        ...
        <div className="table-wrapper">
          <div style={{ marginBottom: 16, textAlign: 'right' }}>
            <Button type="primary" icon={<PlusOutlined />} onClick={() => {showModal('create')}}>新增厂区</Button>
          </div>
          <Table
      ...
      <Modal title="新增/修改厂区" open={isModalOpen} onOk={handleOk} onCancel={handleCancel} confirmLoading={loadingOk}>
        <Form
          labelCol={{ span: 5 }}
          style={{ padding: 16 }}
          form={formCreate}
        ...
        >
          <Form.Item label="厂区名称" name="areName"
            rules={[
              {
                required: true,
                message: '请输入',
          ...
          <Form.Item label="厂区编码" name="areSn"
            rules={[
              {
                required: true,
                message: '请输入',
