...
const App = () => {
  ...
  const handleOk = () => {
    formCreate.validateFields().then(values => {
      ...
      setLoadingOk(true)
      const { program, mainItem, replaceItem } = values
      
      http.post(config.API_PREFIX + api.itemReplaceProgram, {
        program,
        mainItem,
        replaceItem,
        isValid: 'Y', // todo 默认咋搞？
  ...
  const save = async (key) => {
    try {
      ...
      if (index > -1) {
        ...
        http.put(config.API_PREFIX + api.itemReplaceProgram, {
          id: item.id,
          ...updatedItem,
        }).then((res) => {
          setData(newData);
  ...
  */
  const del = (record) => {
    confirm({
      ...
      onOk() {
        http.del(config.API_PREFIX + api.itemReplaceProgram + `/${record.id}`, {}).then((res) => {
          fetchData()
  ...
  const fetchData = () => {
    ...
    const {program, mainItem, replaceItem} = formSearch.getFieldsValue()
    if (program) {
    ...
    http.post(config.API_PREFIX + api.itemReplaceProgramPage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  const loadTemplate = () => {
    ...
    http.post(config.API_PREFIX + api.itemReplaceProgramLoadTemplate, {
    }).then((res) => {
  ...
  const handleCustomRequest = ({ file, onSuccess, onError }) => {
    ...
    http.post(config.API_PREFIX + api.itemReplaceProgramImportData, formData).then((res) => {
  ...
  const exportData = () => {
    ...
    http.post(config.API_PREFIX + api.itemReplaceProgramExportData + `?${query}`, ).then((res) => {
      message.success('导出成功！')
      downloadCSV(res, '程序替代料导出-CSV文件')
          ...
          <Form form={formSearch} onFinish={onFinish}>
            <Row gutter="24">
              <Col span={8}>
                ...
                <Form.Item label="程序名" name="program">
      ...
      <Modal confirmLoading={loadingOk} title="新增程序替代料" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
        ...
        >
          <Form.Item
            label="程序"
            name="program"
            rules={[{ required: true, message: '请输入程序' }]}
          >
            <Input placeholder="请输入程序" />
          </Form.Item>
          <Form.Item
            label="主料号"
            name="mainItem"
