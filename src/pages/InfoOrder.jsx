...
const App = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    {
      title: '消耗数量',
      dataIndex: 'totalConsumptionQty',
      key: 'totalConsumptionQty',
      width: 100,
    },
    {
      title: '贴装数量',
    ...
    {
      title: '吸取错误',
      dataIndex: 'pickErrorQty',
      key: 'pickErrorQty',
      width: 100,
    },
    {
      title: '影像错误',
      dataIndex: 'identErrorQty',
      key: 'identErrorQty',
      width: 100,
    },
    {
      title: '其他错误',
      dataIndex: 'otherErrorQty',
      key: 'otherErrorQty',
      width: 100,
    },
    {
  ...
  const { components, resizableColumns, tableWidth, resetColumns } = useAntdResizableHeader({
    ...
    columnsState: {
      persistenceKey: 'localKeyInfoOrder',
  ...
  const fetchData = () => {
    ...
    const {factoryId, workshopId, areaId, programName, projectNo, itemCode, itemSn, startTime, endTime} = formSearch.getFieldsValue()
    if (factoryId) {
    ...
    http.post(config.API_PREFIX + api.projectItemPage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  const exportData = () => {
    ...
    http.post(config.API_PREFIX + api.projectItemExportData + `?${query}`, ).then((res) => {
      message.success('导出成功！')
      downloadCSV(res, '信息查询-导出工单物料-CSV文件')
...
*/
  ...
  const saveColumnsOrder = (columns) => {
    localStorage.setItem('columnsOrderInfoOrder', JSON.stringify(columns.map(col => col.key)));
  };

  useEffect(() => {
    const savedOrder = localStorage.getItem('columnsOrderInfoOrder');
    if (savedOrder) {
  ...
  function refreshPage() {
    localStorage.removeItem('columnsOrderInfoOrder')
