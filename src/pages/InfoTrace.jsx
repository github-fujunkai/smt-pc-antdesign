...
const App = () => {
  ...
  */
  ...
  const { components, resizableColumns, tableWidth, resetColumns } = useAntdResizableHeader({
    ...
    columnsState: {
      persistenceKey: 'localKeyInfoTrace',
  ...
  const fetchData = () => {
    ...
    const {factoryId, workshopId, areaId, programName, itemCode,boardNum, supplierName, batch, itemSn, startTime, endTime} = formSearch.getFieldsValue()
    if (factoryId) {
    ...
    http.post(config.API_PREFIX + api.deviceMaterialTracePage + `?${qs.stringify(params)}`, {}).then((res) => {
  ...
  const exportData = () => {
    ...
    http.post(config.API_PREFIX + api.deviceMaterialTraceExportData + `?${query}`, ).then((res) => {
      message.success('导出成功！')
      downloadCSV(res, '信息查询-导出追溯信息-CSV文件')
...
*/
  ...
  const saveColumnsOrder = (columns) => {
    localStorage.setItem('columnsOrderInfoTrace', JSON.stringify(columns.map(col => col.key)));
  };

  useEffect(() => {
    const savedOrder = localStorage.getItem('columnsOrderInfoTrace');
    if (savedOrder) {
  ...
  function refreshPage() {
    localStorage.removeItem('columnsOrderInfoTrace')
        ...
        <div className="search-wrapper" style={{display: isShowSearch ? 'block' : 'none'}}>
          ...
          >
            <Row gutter="24">
              ...
              <Col span={8}>
                <Form.Item label="制令单" name="projectNo">
                  <Input
                    placeholder="请输入"
                    allowClear
                  />
              ...
              <Col span={8}>
                <Form.Item label="供应商" name="supplierName">
                  <Select
                    placeholder="请选择"
                    allowClear
                    options={dictBaseAll?.supplierName?.map(item => ({
                      value: item.dictKey,
                      label: item.dictValue,
                    }))}
                  />
              ...
              <Col span={8}>
                <Form.Item label="批次" name="batch">
                  <Select
                    placeholder="请选择"
                    allowClear
                    options={dictBaseAll?.batch?.map(item => ({
                      value: item.dictKey,
                      label: item.dictValue,
                    }))}
                  />
              ...
              <Col span={8}>
                <Form.Item label="料盘UID" name="itemSn">
                  <Input
                    placeholder="请输入"
                    allowClear
                  />
